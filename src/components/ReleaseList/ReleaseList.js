...
class ReleaseList extends React.PureComponent {
  ...
  renderMessage() {
    ...
    if (this.props.loading) {
      text = (
        <span>
          Retrieving releases of <b>{this.props.repoName}</b>
        </span>
      );
    } else if (this.props.releases.length === 0) {
      text = (
        <span>
          <b>{this.props.repoName}</b> doesn't have any public release
        </span>
      );
    } else {
      text = (
        <span>
          Showing <b>{this.props.releases.length}</b> releases of{" "}
          <b>{this.props.repoName}</b>
        </span>
      );
    }
